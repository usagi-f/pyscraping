...
def img_url_list(word):
    url = 'https://twitter.com/{}/media?lang=ja'.format(quote(word))
    byte_content, _ = fetcher.fetch(url)
    structured_page = BeautifulSoup(byte_content.decode('UTF-8'), 'html.parser')
    img_link_elems = structured_page.find_all('div', attrs={'class': 'AdaptiveMedia-photoContainer'})
    img_urls = [e.get('data-image-url') for e in img_link_elems if e.get('data-image-url').startswith('http')]
